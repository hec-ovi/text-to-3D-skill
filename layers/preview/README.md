...
**Remove** in the viewer footer deletes the loaded asset, behind a confirm step whose button is somewhere else, so a double click on Remove cannot delete anything. It takes the whole asset: the GLB, the mesh it superseded, and the render it came from. This is the only route in the layer that writes to the served directory.
...
- **The turntable draws on demand, not every frame.** It used to run occlusion, bloom and a multisampled resolve sixty times a second to produce the same still image sixty times. Something has to ask for a frame now: a control, a load, a resize, or damping still settling. Auto rotate and a playing clip ask continuously, which is correct, and everything else is free. Add a control that changes what is on screen without setting `needsFrame` and your change will appear only when something else happens to redraw.
